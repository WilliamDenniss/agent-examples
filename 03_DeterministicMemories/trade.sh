...
# Wait for async memory generation to complete
echo ""
echo "Waiting 30s for memory generation..."
sleep 30

# Follow-up: create a _new_ session with the same user to test memory recall
echo ""
echo "Creating new session (same user)..."
SESSION_RESPONSE2=$(curl -s -X POST \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  "${BASE_URL}:query" \
  -d "{\"classMethod\":\"create_session\",\"input\":{\"user_id\":\"${USER_ID}\"}}")

SESSION_ID2=$(echo "${SESSION_RESPONSE2}" | jq -r '.output.id')
echo "New session created: ${SESSION_ID2}"

FOLLOWUP="What did you recently trade?"
echo "Sending follow-up query: ${FOLLOWUP}"
curl -s -X POST \
  -H "Authorization: Bearer ${TOKEN}" \
  -H "Content-Type: application/json" \
  "${BASE_URL}:streamQuery" \
  -d "{\"classMethod\":\"stream_query\",\"input\":{\"user_id\":\"${USER_ID}\",\"session_id\":\"${SESSION_ID2}\",\"message\":\"${FOLLOWUP}\"}}" \
  | jq -rj 'select(.content.parts) | .content.parts[] | select(.text) | .text'
