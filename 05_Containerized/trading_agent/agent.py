...
async def save_to_memory(callback_context: CallbackContext):
    # Persisting to memory is best-effort: when no memory store is configured
    # (e.g. running locally or in a container without a Memory Bank), the memory
    # calls below raise. Log and carry on rather than failing the trading cycle.
    try:
        ...
        print(f"No memory store available; skipping memory save: {e}", flush=True)
